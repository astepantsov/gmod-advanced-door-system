...
net.Receive("advdoors_updaterent", function(len, ply)
	...
	if (AdvDoors.getOwner(data.door) == ply) then
		if isbool(data.canRent) and isnumber(data.rentPrice) and data.rentPrice >= 1 and isnumber(data.rentLength) and data.rentLength >= 1 and data.rentLength <= 60 and isnumber(data.rentMaxPeriods) and data.rentMaxPeriods >= 1 then
